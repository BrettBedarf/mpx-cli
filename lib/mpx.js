...
program
    ...
    .action(async (source, destination, options) => {
        try {
            ...
            // Handle stdin input using dash convention
            if (source === "-" || source === "/dev/stdin" || source === "/proc/self/fd/0") {
                ...
                if (inputBuffer.length === 0) {
                    console.log("No data received from stdin");
                    return process.exit(1);
                }

                debugLog("Received", inputBuffer.length, "bytes from stdin");
            }

            let sourceExt;
            if (source === "-" || source === "/dev/stdin" || source === "/proc/self/fd/0") {
                // Detect extension from buffer content using magic bytes
                sourceExt = convert.GetExtensionFromBuffer(inputBuffer);
            } else {
            ...
            debugLog("Source:", source === "-" ? "stdin" : source);
            debugLog("Destination:", destination);
            debugLog("Source extension:", sourceExt);
          ...
          (destinationExt === ".mmd" || destinationExt === ".md"):
                await convert.ImageToMMD(source, destination, inputBuffer);
                break;
            case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".html":
                await convert.ImageToHTML(source, destination, inputBuffer);
                break;
            case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".docx":
                await convert.ImageToDOCX(source, destination, inputBuffer);
                break;
            case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".tex":
                await convert.ImageToTEX(source, destination, inputBuffer);
                break;
            case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".pdf":
                if (options.pdfMethod === "html") {
                    await convert.ImageToHTMLPDF(source, destination, inputBuffer);
                    break;
                } else {
                    await convert.ImageToLatexPDF(source, destination, inputBuffer);
                    break;
                }
                break;
            ...
            console.log(`Converted ${source === "-" ? "stdin" : source} to ${destination} in ${Math.round(processDurationMS)}ms`);
        } catch (error) {
            console.log(
