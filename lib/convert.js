...
function GetExtensionFromBuffer(buffer) {
    ...
    // These are standardized byte sequences at the start of files that identify the format
    if (buffer.length >= 4) {
        const header = buffer.slice(0, 4);
        // PNG: 0x89 0x50 0x4E 0x47 (0x89 followed by "PNG" in ASCII)
        if (header[0] === 0x89 && header[1] === 0x50 && header[2] === 0x4E && header[3] === 0x47) {
            return ".png";
        // JPEG: 0xFF 0xD8 (JPEG/JFIF marker)
        } else if (header[0] === 0xFF && header[1] === 0xD8) {
            return ".jpg";
        // PDF: 0x25 0x50 0x44 0x46 ("%PDF" in ASCII)
...
async function ConvertImageWithSnipAPI(authToken, outputType, source, destination, htmlPDF, sourceBuffer) {
    process.stdout.write(`Converting image to ${outputType}\r`);

    const formData = new FormData();
    
    if (sourceBuffer) {
        // Use buffer directly from stdin
        formData.append("file", sourceBuffer, { 
            filename: `image${GetExtensionFromBuffer(sourceBuffer)}`,
            contentType: GetContentTypeFromBuffer(sourceBuffer)
        });
    } else {
        // Use file stream for file input
        formData.append("file", fs.createReadStream(source));
    }
...
async function ConvertImageWithOCRAPI(apiKey, outputType, source, destination, htmlPDF, sourceBuffer) {
    process.stdout.write(`Converting image to ${outputType}\r`);
...
async function ConvertImage(outputType, source, destination, htmlPDF, sourceBuffer) {
    await config.checkEnvironmentVariables();
    if (process.env["MATHPIX_OCR_API_KEY"]) {
        console.log("Convert with ocr api", source, destination, outputType);
        await ConvertImageWithOCRAPI(process.env["MATHPIX_OCR_API_KEY"], outputType, source, destination, htmlPDF, sourceBuffer);
    } else {
...
async function ImageToMMD(source, destination, sourceBuffer) {
    await ConvertImage("mmd", source, destination, false, sourceBuffer);
}

async function ImageToDOCX(source, destination, sourceBuffer) {
    await ConvertImage("docx", source, destination, false, sourceBuffer);
}

async function ImageToTEX(source, destination, sourceBuffer) {
    await ConvertImage("tex", source, `${destination}.zip`, false, sourceBuffer);
}

async function ImageToHTML(source, destination, sourceBuffer) {
    await ConvertImage("html", source, destination, false, sourceBuffer);
}

async function ImageToLatexPDF(source, destination, sourceBuffer) {
    await ConvertImage("pdf", source, destination, false, sourceBuffer);
}

async function ImageToHTMLPDF(source, destination, sourceBuffer) {
    await ConvertImage("pdf", source, destination, true, sourceBuffer);
}
